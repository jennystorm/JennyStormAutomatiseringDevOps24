...
- Jag får information om vad som finns i mitt inventory, vilka grupper, hosts och IP-adresser. I mitt fall, mina två VM:s webserver och dbserver.

## QUESTION B

What happens if you run `ansible-inventory --graph` in the directory you created above?
- Jag får ungefär samma information som ovan, grupper i mitt inventory och vilka IP-adresser som är kopplade till de grupperna. Men i en annan sorts trädstruktur istället.
...
The command `ansible-config` can be used for creating, viewing, validating, and listing Ansible configuration.
